...
def change_price(val, adj):
    ...
    # val["label"] = val["is_rised"].where(val["is_rised"] is True, 1)
    # val["label"] = val["is_falled"].where(val["is_falled"] is True, 2)
    # print(f"{val=}")
    # val[val["is_rised"], "label"] = 1
    # val[val["is_falled"], "label"] = 2
    # val[val["is_zero"], "label"] = 0
    # val["label"] = 0
    # val.loc[val["limit"] >= 80, "label"] = 100
    # val.loc[val["is_zero"] == True, "label"] = 0
    # val.loc[val["is_rised"] == True, "label"] = 1
    # val.loc[val["is_falled"] == True, "label"] = 2
...
class JStocksDataset(Dataset):
    ...
    def __init__(self):
        ...
        prices = prices.drop(["is_rised", "is_zero", "is_falled", "label"], axis=1)
        ...
        self.label = torch.tensor(
            tmp_label.iloc[: -self.TEST_SIZE].values.astype(np.long)
        )
